...
RadialMenu.prototype.createHolder = function () {
  ...
  var top = document.getElementById('drag-content')
  top.appendChild(self.holder)
  /* var text = document.createElement("div");
    text.setAttribute('class', 'textCon');
    text.innerHTML = '关闭';
    text.style.textAlign="center";
    self.holder.appendChild(text) */
}
/// /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/// /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
RadialMenu.prototype.handleCenterClick = function () { // 控制中间内容不能点击
  var self = this
...
// 返回父菜单
RadialMenu.prototype.goParentMenu = function (content) {
  var self = this
  ...
  if (self.parentItems.length > 0) {
    ...
    RadialMenu.setClassAndWaitForTransition(self.currentMenu, 'menu inner').then(function () {
      ...
      self.updateCenter(self.currentMenu, content, 8)
    })
  } else {
    self.updateCenter(self.currentMenu, content, 8)
  }
...
// 更新选择元素信息
RadialMenu.prototype.updateCenter = function (svg, content, size) {
  var self = this
  let ele = svg.getElementById('Info')
  if (ele) {
    svg.removeChild(ele)
    ...
    var text = self.createText(0, 0, content)
    g.appendChild(text)
    svg.appendChild(g)
  }
}
...
RadialMenu.prototype.createCenter = function (svg, title, icon, size) {
  var self = this
  /* 中间文本内容 */
  var textCon = '<tspan  x="0" y="-6">请选择</tspan><br/>' +
                  '<tspan  x="0" y="2">页面元素</tspan><br/>'
                  /* '<tspan  x="0" y="9">请操作</tspan>' */
  ...
  if (title === 'Info') { // 判断中间内容是否显示文本
    var text = self.createText(0, 0, textCon)
    g.appendChild(text)
  } else if (title === 'Return') { // 判断中间内容是否显示图标
...
RadialMenu.prototype.createMenu = function (classValue, levelItems, nested) {
  ...
  if (nested) {
    self.createCenter(svg, 'Return', '#return', 8)
  } else {
    self.createCenter(svg, 'Info', '#close', 7)
  }
